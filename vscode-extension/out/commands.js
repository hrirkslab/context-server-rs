...
class CommandManager {
    ...
    async executeSuggestionAction(params) {
        try {
    ...
    async handleCreateContextAction(suggestion, action) {
    ...
    async handleShowDocumentationAction(suggestion, action) {
        // Create a webview to show documentation
        const panel = vscode.window.createWebviewPanel('contextDocumentation', `Documentation: ${suggestion.title}`, vscode.ViewColumn.Beside, {
            enableScripts: true,
            retainContextWhenHidden: true
        });
        panel.webview.html = this.getDocumentationWebviewContent(suggestion);
    }
    async handleRunAnalysisAction(suggestion, action) {
        const activeEditor = vscode.window.activeTextEditor;
        if (!activeEditor) {
            vscode.window.showInformationMessage('No active editor found');
            return;
    ...
    async handleApplyFixAction(suggestion, action) {
        vscode.window.showInformationMessage('Automated fix functionality will be implemented in a future version');
    }
    async showSuggestionDetails(suggestion) {
        const panel = vscode.window.createWebviewPanel('suggestionDetails', `Suggestion: ${suggestion.title}`, vscode.ViewColumn.Beside, {
            enableScripts: true,
            retainContextWhenHidden: true
        });
        panel.webview.html = this.getSuggestionDetailsWebviewContent(suggestion);
    }
    async configureAnalysisRules() {
    ...
    async addAnalysisRule() {
        ...
        if (!name)
            return;
        const language = await vscode.window.showQuickPick(this.configManager.getSupportedLanguages(), { placeHolder: 'Select target language' });
        if (!language)
            return;
    ...
    async viewAnalysisRules() {
        ...
        if (selected) {
            const panel = vscode.window.createWebviewPanel('analysisRuleDetails', `Analysis Rule: ${selected.rule.name}`, vscode.ViewColumn.Beside, { enableScripts: false });
            panel.webview.html = this.getAnalysisRuleWebviewContent(selected.rule);
        }
    }
    async removeAnalysisRule() {
        const rules = this.configManager.getAnalysisRules();
        if (rules.length === 0) {
        ...
        if (selected) {
            const confirm = await vscode.window.showWarningMessage(`Are you sure you want to remove the rule "${selected.rule.name}"?`, 'Yes', 'No');
            if (confirm === 'Yes') {
    ...
    async configureSuggestionTriggers() {
        const action = await vscode.window.showQuickPick([
            'Add New Trigger',
    ...
    async viewSuggestionTriggers() {
        ...
        if (selected) {
            const panel = vscode.window.createWebviewPanel('suggestionTriggerDetails', `Suggestion Trigger: ${selected.trigger.name}`, vscode.ViewColumn.Beside, { enableScripts: false });
            panel.webview.html = this.getSuggestionTriggerWebviewContent(selected.trigger);
        }
    }
    async removeSuggestionTrigger() {
        const triggers = this.configManager.getSuggestionTriggers();
        if (triggers.length === 0) {
        ...
        if (selected) {
            const confirm = await vscode.window.showWarningMessage(`Are you sure you want to remove the trigger "${selected.trigger.name}"?`, 'Yes', 'No');
            if (confirm === 'Yes') {
    ...
    async showContextHealth() {
        try {
            const health = await this.contextClient.getContextHealth();
            const panel = vscode.window.createWebviewPanel('contextHealth', 'Context Health Report', vscode.ViewColumn.Beside, {
                enableScripts: true,
                retainContextWhenHidden: true
            });
    ...
    async showProjectInsights() {
        try {
            const insights = await this.contextClient.getProjectInsights();
            const panel = vscode.window.createWebviewPanel('projectInsights', 'Project Insights', vscode.ViewColumn.Beside, {
                enableScripts: true,
                retainContextWhenHidden: true
            });
    ...
    async queryContext() {
        ...
        try {
            ...
            const panel = vscode.window.createWebviewPanel('contextQueryResults', `Query Results: ${query}`, vscode.ViewColumn.Beside, {
                enableScripts: true,
                retainContextWhenHidden: true
            });
    ...
    async toggleRealTimeSuggestions() {
    ...
    getDocumentationWebviewContent(suggestion) {
        return `
            <!DOCTYPE html>
            <html>
            <head>
            ...
            <body>
                ...
                <div class="content">
                    ...
                    ${Object.keys(suggestion.metadata).length > 0 ? `
                        ...
                        <div class="metadata">
                            ${Object.entries(suggestion.metadata).map(([key, value]) => `<strong>${key}:</strong> ${value}<br>`).join('')}
                        </div>
                    ` : ''}
                </div>
            </body>
    ...
    getSuggestionDetailsWebviewContent(suggestion) {
        return `
            <!DOCTYPE html>
            <html>
            <head>
            ...
            <body>
                ...
                <div class="content">
                    ...
                    ${Object.keys(suggestion.metadata).length > 0 ? `
                        <h2>Metadata</h2>
                        <div class="metadata">
                            ${Object.entries(suggestion.metadata).map(([key, value]) => `<strong>${key}:</strong> ${JSON.stringify(value)}<br>`).join('')}
                        </div>
                    ` : ''}
                </div>
    ...
    getAnalysisRuleWebviewContent(rule) {
        return `
            <!DOCTYPE html>
            <html>
            <head>
    ...
    getSuggestionTriggerWebviewContent(trigger) {
        return `
            <!DOCTYPE html>
            <html>
            <head>
    ...
    getContextHealthWebviewContent(health) {
        return `
            <!DOCTYPE html>
            <html>
            <head>
    ...
    getProjectInsightsWebviewContent(insights) {
        return `
            <!DOCTYPE html>
            <html>
            <head>
            ...
            insights.popular_context_types.map((type) => `
                            <div class="metric">
    ...
    getQueryResultsWebviewContent(query, results) {
        return `
            <!DOCTYPE html>
            <html>
            <head>
