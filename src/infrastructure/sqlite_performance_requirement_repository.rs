...
impl PerformanceRequirementRepository for SqlitePerformanceRequirementRepository {
    async fn create(&self, requirement: &PerformanceRequirement) -> Result<PerformanceRequirement, McpError> {
    ...
    async fn find_by_project_id(&self, project_id: &str) -> Result<Vec<PerformanceRequirement>, McpError> {
        let db = self.db.lock().unwrap();
        ...
        let requirement_rows = stmt.query_map([project_id], |row| {
    ...
    async fn find_by_id(&self, id: &str) -> Result<Option<PerformanceRequirement>, McpError> {
        ...
        let mut requirement_iter = stmt.query_map([id], |row| {
            Ok(PerformanceRequirement {
                id: row.get(0)?,
                project_id: row.get(1)?,
                component_area: row.get(2)?,
    ...
    async fn update(&self, requirement: &PerformanceRequirement) -> Result<PerformanceRequirement, McpError> {
        let db = self.db.lock().unwrap();
    ...
    async fn delete(&self, id: &str) -> Result<bool, McpError> {
        ...
        let rows_affected = db.execute("DELETE FROM performance_requirements WHERE id = ?", [id])
            .map_err(|e| McpError::internal_error(format!("Database error: {}", e), None))?;
