...
impl FrameworkRepository for SqliteFrameworkRepository {
    ...
    async fn find_by_project_id(&self, project_id: &str) -> Result<Vec<FrameworkComponent>, McpError> {
        ...
        let component_iter = stmt.query_map([project_id], |row| {
            ...
            McpError::internal_error(format!("Failed to query framework components: {}", e), None)
        )?;

        let mut components = Vec::new();
        for component in component_iter {
            components.push(component.map_err(|e| 
                McpError::internal_error(format!("Failed to parse framework component: {}", e), None)
            )?);
        }
    ...
    async fn find_by_id(&self, id: &str) -> Result<Option<FrameworkComponent>, McpError> {
        ...
        let mut component_iter = stmt.query_map([id], |row| {
            ...
            McpError::internal_error(format!("Failed to query framework component: {}", e), None)
        )?;
    ...
    async fn delete(&self, id: &str) -> Result<bool, McpError> {
        ...
        let rows_affected = db.execute(
            "DELETE FROM framework_components WHERE id = ?1",
            [id],
        ).map_err(|e| 
            McpError::internal_error(format!("Failed to delete framework component: {}", e), None)
        )?;
    ...
    async fn find_by_architecture_layer(&self, project_id: &str, layer: &str) -> Result<Vec<FrameworkComponent>, McpError> {
        ...
        let component_iter = stmt.query_map([project_id, layer], |row| {
            let dependencies_str: String = row.get(6)?;
            let dependencies: Vec<String> = serde_json::from_str(&dependencies_str).unwrap_or_default();
            ...
            McpError::internal_error(format!("Failed to query framework components by layer: {}", e), None)
        )?;
